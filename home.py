...
pages = []
for i in range(7):
    pages.append(tk.Frame(root, bg="#4D96FF"))
...
def goBack():
    ...
    if (active_page==7 and not thread_event.is_set()):
        thread_event.set()
        webcam.release()
...
def basicPageSetup(pageNo):
    ...
    heading = tk.Label(pages[pageNo], fg="black", bg="#4D96FF", font="Arial 20 bold", pady=10)
    heading.pack()

    content = tk.Frame(pages[pageNo], bg="#4D96FF", pady=20)
    content.pack(expand="true", fill="both")

    left_frame = tk.Frame(content, bg="#4D96FF")
...
def getPage1():
    ...
    basicPageSetup(1)
    heading.configure(text="Register Criminal", bg="#4D96FF")
    right_frame.configure(text="Enter Details", fg="white", bg="#4D96FF")

    btn_grid = tk.Frame(left_frame, bg="#4D96FF")
    btn_grid.pack()
    ...
    canvas = tk.Canvas(right_frame, bg="#4D96FF", highlightthickness=0)
    canvas.pack(side="left", fill="both", expand="true", padx=30)
    scrollbar = tk.Scrollbar(right_frame, command=canvas.yview, width=20, troughcolor="#4D96FF", bd=0,
                          activebackground="#4D96FF", bg="#000000", relief="raised")
    scrollbar.pack(side="left", fill="y")

    scroll_frame = tk.Frame(canvas, bg="#4D96FF", pady=20)
    ...
    tk.Label(scroll_frame, text="* Required Fields", bg="#4D96FF", fg="#F90716", font="Arial 13 bold").pack()
    # Adding Input Fields
...
def getPage4(path):
    ...
    btn_grid = tk.Frame(right_frame, bg="#3E3B3E")
    btn_grid.pack()
...
# Define thread_event as a threading.Event object
thread_event = threading.Event()

def stop_live_surveillance():
    global thread_event

    if thread_event[0]:
        thread_event[0] = False


def start_live_recognition(model, names):
        global thread_event, left_frame, webcam, img_label
        start = time.time()
...
def getPage5():
    ...
    tk.Button(btn_grid, text="Start Surveillance", command=getPage6, font="Arial 15 bold", padx=20, bg="#000000",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#3E3B3C",
              activeforeground="white").grid(row=0, column=0, padx=10, pady=25)
    tk.Button(btn_grid, text="Stop Surveillance", command=stop_live_surveillance, font="Arial 15 bold", padx=20,
              bg="#000000",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#3E3B3C",
              activeforeground="white").grid(row=0, column=1, padx=25, pady=25)
...
def getPage6():
    ...
    thread_event = threading.Event()
    thread = threading.Thread(target=start_live_recognition, args=(model, names))
    thread.start()
...
######################################## Home Page ####################################
tk.Label(pages[0], text="Criminal Detection System", fg="black", bg="#4D96FF",
      font="Arial 50 bold", pady=30).pack()
imge = Image.open("Capture1.png")
res_image = imge.resize((500, 340))
logo = ImageTk.PhotoImage(res_image)
tk.Label(pages[0], image=logo, bg="#4D96FF").pack(side='bottom')

btn_frame = tk.Frame(pages[0], bg="#4D96FF", pady=100)
btn_frame.pack()



tk.Button(btn_frame, text="Add Criminal Details", command=getPage1).grid(row=0, column=0, padx=25, pady=25)
tk.Button(btn_frame, text="Image Surveillance", command=getPage2).grid(row=0, column=1, padx=25, pady=25)
tk.Button(btn_frame, text="Video Surveillance", command=getPage3).grid(row=1, column=0, padx=25, pady=25)
tk.Button(btn_frame, text="Live Surveillance", command=getPage5).grid(row=1, column=1, padx=25, pady=25)


for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=17, bg="#000000", fg="white",
        pady=8, bd=6, activebackground="#3E3B3C", activeforeground="white")
